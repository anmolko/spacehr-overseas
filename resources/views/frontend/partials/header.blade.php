...
<head>
    ...
    <meta name="description" content="{{ucwords(@$setting_data->meta_description ?? 'Space HR solution ')}} "/>
    <meta name="keywords" content="{{@$setting_data->meta_tags ?? 'Space HR solution '}}">
    <meta name="csrf-token" content="{{ csrf_token() }}" />
    <link rel="canonical" href="https://spacehrsolution.com" />

    @if (\Request::is('/'))
        <title>{{ucwords(@$setting_data->website_name ?? 'Space HR solution')}}</title>
    @else
        <title>@yield('title') | {{ucwords(@$setting_data->website_name ?? 'Space HR solution')}} </title>
    @endif

    <meta property="og:title" content=" {{ucwords(@$setting_data->meta_title ?? 'Space HR solution')}}" />
    ...
    <meta property="og:description" content=" {{ucwords(@$setting_data->meta_description ?? 'Space HR solution')}}" />
